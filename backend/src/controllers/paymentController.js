...
const paymentBodyValidator = require("../validators/paymentValidator");
const { getPassword, getTimestamp } = require("../utils/paymentUtils");
...
function lipa_na_mpesa(req, res, next) {
  ...
  let data = {
    ...
    AccountReference: "Masomo",
    TransactionDesc: "Masomo Service",
  };
  axios
    .post(stkUrl, data, {
      headers: {
  ...
  try {
    ...
    const queryTransactionStatus = async () => {
      try {
        ...
        console.error(
          "Error response from Safaricom API:",
          error.response.data
        );
        if (
          retries < MAX_RETRIES &&
